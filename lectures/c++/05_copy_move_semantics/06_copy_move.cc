...
class Vector {
 ...
 public:
  ...
  // move ctor           
 // r value reference (&&) -> is a variable that can stay both on the left and on the right side of an equal  (this object dies at the end of the line)
 // no put 'const' befor a r value variable
  Vector(Vector&& v) : _size{std::move(v._size)}, elem{std::move(v.elem)} {
    std::cout << "move ctor\n";
  }

  // Vector(Vector&& v) = default; // ok     it is ok because we have unique pointers, but it will not works as longer as we use rough pointers (builting in types)
...
Vector<T>::Vector(const Vector& v) : _size{v._size}, elem{new T[_size]} {
  std::cout << "copy ctor\n";
  std::copy(v.begin(), v.end(), begin());    // copy is defines in algorithm
}
