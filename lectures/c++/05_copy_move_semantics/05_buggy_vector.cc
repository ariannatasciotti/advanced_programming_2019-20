...
template <typename T>
class Vector {
  std::unique_ptr<T[]> elem;     // get(), reset(), release() functions for unique pointers.. i'm the only one that own this vector, so nobody can copy this.. delete on copy assignment and on copy constructor (automatically)
  std::size_t _size;
