...
// i want to do different things depends on which type in printed.. but how can I do this if I will know the type only at run time? Use dynamic_cast
void print_animal(const Animal& a) noexcept {
  a.info();
  a.speak();

  // dynamic_cast is used to know the real type of the object
  if (dynamic_cast<const DangerousSnake*>(&a))   // it is useful when you want to performe different things for different types
    std::cout << "call 911...\n";
}
